...
def output_results(df: pd.DataFrame,filename:str):
...
def output_results2(df: pd.DataFrame):
    if not os.path.exists('results'):
        os.makedirs('results')
    path = 'results/dailylist.csv'
    df.to_csv(path, header=True)
    return path
...
def trade_marked(premium_index_df:pd.DataFrame,number: int,interval_days:int):
    """
    ...
    print("     Maximum holdings = %d STOCKS\n"%(number))
    ...
    for name,group in daily_trade_group:
        ...
        daily_premium_list[i] = daily_premium_list[i][:20]
    ...
    for j in range(0,len(daily_trade_group),1):
            if (j==0):
                ...
                daily_sell_list[j] = list(set(daily_holding_list[j]).difference(set((daily_full_list[j+1])))) #非换仓日，检查持仓如果不在明天Full list里，则不能再持有下去，则卖出
                daily_buy_list[j] = [] #非换仓日，不做买入交易
                daily_holding_list[j] =  list(set(daily_holding_list[j] + daily_buy_list[j])-set(daily_sell_list[j]))
            else:
                daily_holding_list[j] = daily_holding_list[j-1] #继承承T-1日的持仓
                daily_sell_list[j] = list(set(daily_holding_list[j]).difference(set((daily_premium_list[j])))) + list(set(daily_holding_list[j]).difference(set((daily_full_list[j+1])))) #换仓日，检查持仓，如果今天不在premium list里，或者不在明天Full List里，则卖出
                daily_buy_list[j] = list(set(daily_premium_list[j]).difference(set((daily_holding_list[j])))) #换仓日，买入premium list里里没有的
                daily_holding_list[j] =  list(set(daily_holding_list[j] + daily_buy_list[j])-set(daily_sell_list[j]))
            ...
            print("         Marking stocks of %s"%(date_list[j].strftime("%Y-%m-%d")))
            #每日更新买入状态（只在换仓日）
            for buy_stock in daily_buy_set:
                temp_buy_df = premium_index_df[(premium_index_df["TRADE_DT"]==date_list[j]) & (premium_index_df["SECUCODE"]==buy_stock)]
            ...
            for sell_stock in daily_sell_set:
                temp_sell_df = premium_index_df[(premium_index_df["TRADE_DT"]==date_list[j]) & (premium_index_df["SECUCODE"]==sell_stock)]
            ...
            for holding_stock in daily_holding_set:
                temp_holdings_df = premium_index_df[(premium_index_df["TRADE_DT"]==date_list[j]) & (premium_index_df["SECUCODE"]==holding_stock)]
...
def equal_value_trade (trade_marked_df:pd.DataFrame,initial_cash:float):
    ...
    for name,group in trade_marked_gp:
        ...
        #最后一日保持持股状态
            ...
            if(group["HOLDINGS"].count()!=0):
                for index,row in group.iterrows():
                    if ((row['HOLDINGS']==1 or row['HOLDINGS']=='1') and not(row['BUY_MARK']==1 or row['BUY_MARK']=='1')):
                        trade_marked_df['HLD_AMT'][index]= stock_holdings_df[stock_holdings_df['SECUCODE']==str(row['SECUCODE'])]['HLD_AMT'].values
                        stock_holdings_df.loc[stock_holdings_df[stock_holdings_df['SECUCODE']==str(row['SECUCODE'])].index[0],'PRICE'] = trade_marked_df['CLOSE'][index]
            
            #先执行卖出操作
            if(group["SELL_MARK"].count()!=0):
                for index,row in group.iterrows():
                 if row['SELL_MARK']==1 or row['SELL_MARK']=='1':
                    hld_amt = float(stock_holdings_df[stock_holdings_df['SECUCODE']==str(row['SECUCODE'])]['HLD_AMT'].values)
                    cash_value[i] = round(cash_value[i] + (row['CLOSE'] * hld_amt),2)
                    #在两张表里的持仓数据清0
                    trade_marked_df['HLD_AMT'][index]= 0
                    stock_holdings_df.drop(index=stock_holdings_df[stock_holdings_df['SECUCODE']==str(row['SECUCODE'])].index[0],inplace=True)
    ...
    print("Trading finieshed.")
    print("Output daily trade records file: daily_trade_records.csv.")
    output_results(trade_marked_df,'daily_trade_records')
...
if __name__ == '__main__':
    ...
    trade_marked_df = trade_marked(premium_df,HOLDING_AMOUNT,INTERVAL_DAYS)
    output_results(trade_marked_df,'trade_marked')
    #trade_marked_df = pd.read_csv('results/trade_marked.csv')
    
    pf_nav_df = equal_value_trade(trade_marked_df,INITIAL_ASSET)
    print("Output daily NAV file: pf_nav.csv.")
    output_results(pf_nav_df,'pf_nav')
